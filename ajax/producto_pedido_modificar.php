...
if($action == 'ajax'){
	...
	if ( isset($_GET['q']) != "" ){
	   ...
	   if ( $_GET['q'] != "" )
	   {
		   ...
		   $sWhere .= ') order by productos.orden asc';
	   }
		...
		$aColumns = array('codigoProducto', 'nombreProducto', 'caja','codigoFamilia','codigoMarca','codigoClase', 'listalinea.codigoLinea', 'codigoListaPrecio','listalinea.nombreLinea');//Columnas de busqueda
		$sTable = "productos,listalinea,clientelinea,clientes";
			...
			<table class="table table-responsive table-striped table-bordered table-sm  fixed_header table-hover" cellspacing="0"  >
				...
				<tbody style=" height:auto; max-height:950px; min-height:auto; width:100%">
					<tr class="" style="background-color:#faebcc!important; ">
						...
						<th  style="text-align:center;" width="10%" class="text-center"><span class="text-center">Cantidad</span></th>
							...
							<tr >
								...
								<td style="text-align:center;"><?php echo utf8_encode($codigoProducto); ?></td>
									...
									<td  style="text-align:center;"><div class="pull-right">
										<input type="number" min="1" onkeyup="el4('<?php echo $id_producto; ?>')"  class="form-control" style="text-align:center" id="cantidad_<?php echo $id_producto; ?>"  value="1" >
									</div></td>
									...
									<td style="width:10%;" class='text-center'><button   class='agregar'href="#" onclick="agregarmodificar('<?php echo $id_producto ?>')"><i  class="glyphicon glyphicon-plus"></i></button></td>
							</tr>
							<?php
						$i=$i+1;
					}
