...
if(isset($_GET['descartar'])){
	$validar=mysqli_query($con,"DELETE FROM tmp WHERE id_factura=".$_GET['id_factura']."");
}
...
if(isset($_GET['id_factura']) && (!isset($_GET['guardar']) && (!isset($_GET['accionar']))==2) && (!isset($_GET['eliminar']))){
	...
	$consultar=mysqli_query($con,"SELECT * FROM tmp WHERE id_factura=".$_GET['id_factura']."");
	$contador=mysqli_num_rows($consultar);
	
	if($contador==0){
	
		$validar=mysqli_query($con,"DELETE FROM tmp WHERE id_factura=".$_GET['id_factura']."");
		$llenardata= mysqli_query($con,"SELECT * FROM detalle_pedido where numero_pedido=".$_GET['id_factura']." order by promocion desc");
		...
		if($count>0){
			...
			while($row= mysqli_fetch_array($llenardata)){
				...
				if($row['promocion']==1){
					$insert_detail=mysqli_query($con, "INSERT INTO tmp(id_factura,id_producto,cantidad_tmp,session_id,precio_tmp,promo,status) VALUES ('$id','$id_producto', '$cantidad','','$precio_tmp',$promocion,1)");
				}else{
					
				
					$seleccionar=mysqli_query($con,"SELECT promocion FROM productos where idProducto=".$id_producto."");
					...
					if($row['promocion']==1){
						...
						$seleccionar=mysqli_query($con,"SELECT id_tmp FROM tmp WHERE id_factura=".$_GET['id_factura']." and promo=1 and verificar=0 limit 1");
						...
						if($seleccion>0){
							...
							if($cantidad>=$_SESSION['numeroUnidBase']){
								$insert_detail=mysqli_query($con, "INSERT INTO tmp(id_factura,id_producto,cantidad_tmp,session_id,precio_tmp,status,ver_promo) VALUES ('$id','$id_producto', '$cantidad','','$precio_tmp',1,1)");
								$update=mysqli_query($con,"UPDATE tmp SET verificar=1 WHERE id_tmp=".$variable['id_tmp']." and promo=1 and id_factura=".$_GET['id_factura']."");
							}else{
								$insert_detail=mysqli_query($con, "INSERT INTO tmp(id_factura,id_producto,cantidad_tmp,session_id,precio_tmp,status,ver_promo) VALUES ('$id','$id_producto', '$cantidad','','$precio_tmp',2,1)");
								$suma=$suma+$cantidad;
								if($suma>=$_SESSION['numeroUnidBase']){
									
									$update=mysqli_query($con,"UPDATE tmp SET verificar=1 WHERE id_tmp=".$variable['id_tmp']." and promo=1 and id_factura=".$_GET['id_factura']."");
									$seleccionar=mysqli_query($con,"SELECT id_tmp FROM tmp WHERE id_factura=".$_GET['id_factura']." and status=2");
									...
									if($seleccion>0){
										while($variable=mysqli_fetch_array($seleccionar)){
											$update=mysqli_query($con,"UPDATE tmp SET status=1 WHERE id_tmp=".$variable['id_tmp']."  and id_factura=".$_GET['id_factura']."");
										}
									}
								}
							}
							...
							$insert_detail=mysqli_query($con, "INSERT INTO tmp(id_factura,id_producto,cantidad_tmp,session_id,precio_tmp,status,ver_promo) VALUES ('$id','$id_producto', '$cantidad','','$precio_tmp',0,1)");
						}	
					}else{
						$insert_detail=mysqli_query($con, "INSERT INTO tmp(id_factura,id_producto,cantidad_tmp,session_id,precio_tmp,status) VALUES ('$id','$id_producto', '$cantidad','','$precio_tmp',1)");
					}
				}
				if (!$insert_detail) {
					$mensaje  = 'Consulta no válida: ' .mysqli_error($con) . "\n";
	...
	if(isset($_GET['id_editar'],$_GET['cantidad_editar'])){
		...
		if($id>0){
			...
			if(!empty(@$_SESSION['promoactiva'])){	
				$eliminardata=mysqli_query($con,"SELECT * FROM tmp WHERE id_tmp=".$id." and id_factura='".$_GET['id_factura']."' ");
					...
					$count=mysqli_num_rows($eliminardata);
						if($count>0){		
							...
							if($sqlupdate['cantidad_tmp']>$cantidad){
								...
								$sumarpromo=mysqli_query($con,"SELECT sum(cantidad_tmp) as cantidad FROM tmp WHERE promo=1 and id_factura='".$_GET['id_factura']."' ");
								...
								if($count>0){		
									$sqlupdate=mysqli_fetch_array($sumarpromo);
										...
										//modificar producto
										$sql=mysqli_query($con,"UPDATE tmp SET cantidad_tmp=".$cantidad." WHERE id_tmp=".$id." and id_factura='".$_GET['id_factura']."' ");
										if (!$sql) {
											$mensaje  = 'Consulta no válida: ' .mysqli_error($con) . "\n";
										...
										do{
											...
											$precio_venta=0;
											$sqlValidarPromo=mysqli_query($con, "select * from tmp where promo=1 and id_factura='".$_GET['id_factura']."'");
											...
											$sql=mysqli_query($con, "select tmp.cantidad_tmp as catidadtotal, productos.capacidad,productos.polaridad, tmp.valor_promo, tmp.disminuir_promo, tmp.status, tmp.id_tmp, tmp.id_producto, tmp.session_id, productos.nombreProducto, tmp.precio_tmp, productos.iva, productos.promocion, productos.codigoLinea, productos.codigoListaPrecio, tmp.promo from productos, tmp where productos.idProducto=tmp.id_producto and productos.promocion=1 and promo!=1 and tmp.id_factura='".$_GET['id_factura']."'  order by tmp.promo desc");
											...
											if($totalvalor<=0){
												$sumarpromo=mysqli_query($con,"SELECT id_tmp FROM tmp WHERE promo=1 and id_factura='".$_GET['id_factura']."' order by cantidad_tmp desc limit 1 ");
												if (!$sumarpromo) {
													$mensaje  = 'Consulta no válida: ' .mysqli_error($con) . "\n";
													$mensaje .= 'Consulta completa: ' . $eliminardata;
												...
												$sql=mysqli_query($con, "select sum(tmp.cantidad_tmp) as cantidadtotal from tmp,productos where productos.idProducto=tmp.id_producto and promo!=1 and productos.promocion=1 and id_factura=".$_GET['id_factura']."");
														...
														if (!$sql) {
															...
															if($row['cantidadtotal']<$_SESSION['numeroUnidBase']){
																	...
																	$sumarpromo=mysqli_query($con,"DELETE FROM tmp WHERE promo=1 and id_factura=".$_GET['id_factura']."");
																	$activar=true;
																...
																do{
																$comprobarnumero=0;
																$sql=mysqli_query($con, "select tmp.cantidad_tmp as cantidadtotal, productos.capacidad,nombreLinea,productos.polaridad, tmp.valor_promo, tmp.disminuir_promo, tmp.status, tmp.id_tmp, tmp.id_producto, tmp.session_id, productos.nombreProducto, tmp.precio_tmp, productos.iva, productos.promocion, productos.codigoLinea, productos.codigoListaPrecio, tmp.promo from productos, tmp, listalinea where productos.idProducto=tmp.id_producto and productos.codigoLinea=listalinea.codigoLinea and id_factura=".$_GET['id_factura']."  order by productos.nombreProducto ASC,tmp.promo DESC");
																...
																if (!$sql) {
																	...
																	if($count>0){
																		...
																		$sqlValidarPromo=mysqli_query($con, "select sum(cantidad_tmp) as cantidad, sum(precio_tmp) as precio from tmp where promo=1 and id_factura=".$_GET['id_factura']."");
																		...
																		if($comprobarnumero<0){
																			$sumarpromo=mysqli_query($con,"DELETE  FROM tmp WHERE promo=1 and id_factura=".$_GET['id_factura']." limit 1");
																			$activar=true;
...
if(isset($_GET['id_factura'],$_GET['id_eliminar'])){
	...
	$activar=null;
	//obtener nombre del productoç
	$nombreProducto=mysqli_query($con,"SELECT nombreProducto FROM productos,tmp where tmp.id_producto=productos.idProducto and  id_tmp=".$_GET['id_eliminar']." and id_factura=".$_GET['id_factura']."");
	//verificar valor precio
	if (!$nombreProducto) {
		$mensaje  = 'Consulta no válida: ' .mysqli_error($con) . "\n";
		$mensaje .= 'Consulta completa: ' . $nombreProducto;
	...
	$nombrerow=mysqli_fetch_array($nombreProducto);
	$verificar=mysqli_query($con,"SELECT * FROM tmp WHERE id_tmp=".$_GET['id_eliminar']." and id_factura=".$_GET['id_factura']."");
	if (!$verificar) {
		$mensaje  = 'Consulta no válida: ' .mysqli_error($con) . "\n";
		$mensaje .= 'Consulta completa: ' . $sql;
		echo $mensaje;
				...
				if(!empty(@$_SESSION['promoactiva'])){	
					...
					if($count>0){		
						$sqlupdate=mysqli_fetch_array($sumarpromo);
							...
							//modificar producto
							$sql=mysqli_query($con,"DELETE FROM tmp WHERE id_tmp=".$_GET['id_eliminar']."");
							if (!$sql) {
								$mensaje  = 'Consulta no válida: ' .mysqli_error($con) . "\n";
								$mensaje .= 'Consulta completa: ' . $sql;
	...
	if(isset($_GET['agregarproducto'])){
		...
		if($_GET['agregarproducto']==1){
			...
			$seleccionar=mysqli_query($con,"SELECT promocion FROM productos where idProducto=".$id_producto."");
				...
				$seleccionar=mysqli_query($con,"SELECT id_tmp,cantidad_tmp,nombreProducto FROM tmp,productos where productos.idProducto=tmp.id_producto and tmp.id_producto=".$id_producto."  and promo!=1 and tmp.id_factura='".$id."' ");
					//verificar valor precio
				if (!$seleccionar) {
					$mensaje  = 'Consulta no válida: ' .mysqli_error($con) . "\n";
					$mensaje .= 'Consulta completa: ' . $seleccionar;
					...
					if($count<=0){
						...
						$row=mysqli_fetch_array($seleccionar);
							if($row['promocion']==1){
								$insert_detail=mysqli_query($con, "INSERT INTO tmp(id_factura,id_producto,cantidad_tmp,session_id,precio_tmp,ver_promo) VALUES ('$id','$id_producto', '$cantidad','','$precio_tmp',1)");
							}else{	
								$insert_detail=mysqli_query($con, "INSERT INTO tmp(id_factura,id_producto,cantidad_tmp,session_id,precio_tmp) VALUES ('$id','$id_producto', '$cantidad','','$precio_tmp')");
							}
							if(!$insert_detail){
			...
			$precio_tmp=$_GET['precio_venta'];
			$consulta=mysqli_query($con,"SELECT * FROM tmp WHERE id_factura=".$id." and cantidad_tmp>=".$_SESSION['numeroUnidBase']." and ver_promo=1 and promo!=1 and status!=1 limit 1 ");
			...
			if($count>0){
				...
				$udpate=mysqli_query($con,"UPDATE tmp SET status=1 WHERE id_tmp=".$query['id_tmp']." and id_factura=".$id."");
				if (!$udpate) {
					$mensaje  = 'Consulta no válida: ' .mysqli_error($con) . "\n";
					$mensaje .= 'Consulta completa: ' . $udpate;
					echo $mensaje;
				}	
				$insert_detail=mysqli_query($con, "INSERT INTO tmp(id_factura,id_producto,cantidad_tmp,session_id,precio_tmp,promo,verificar,status) VALUES ('$id','$id_producto', '$cantidad','','$precio_tmp',1,1,1)");
				if (!$insert_detail) {
					$mensaje  = 'Consulta no válida: ' .mysqli_error($con) . "\n";
					$mensaje .= 'Consulta completa: ' . $insert_detail;
					echo '<script> M.toast({html: "Error en almacenar el producto con la ID: '.$_GET['id'].'",classes: "error",displayLength:4000,inDuration:700,outDuration:700});</script>';
				...
				$query=mysqli_query($con,"SELECT sum(cantidad_tmp) as cantidad FROM tmp WHERE ver_promo=1 and status=0 and id_factura=".$id."");
				if (!$query) {
					$mensaje  = 'Consulta no válida: ' .mysqli_error($con) . "\n";
				...
				if(($count=mysqli_num_rows($query))>0){
					...
					if($sql['cantidad']>=$_SESSION['numeroUnidBase']){
						//echo 'aqui';
						$query=mysqli_query($con,"SELECT * FROM tmp WHERE ver_promo=1 and status=0 and id_factura=".$id."");
						while($row=mysqli_fetch_array($query)){
							$udpate=mysqli_query($con,"UPDATE tmp SET status=1 WHERE id_tmp=".$row['id_tmp']." and id_factura=".$id."");
							if (!$udpate) {
								$mensaje  = 'Consulta no válida: ' .mysqli_error($con) . "\n";
								$mensaje .= 'Consulta completa: ' . $udpate;
								echo $mensaje;
...
if(isset($_GET['guardar'])){
		...
		if ($count>0){
			...
			if (!$insert_detail) {
				...
				$insert_tmp=mysqli_query($con, "UPDATE pedido set fecha_pedido='".$fecha."', hora_creacion='".$hora_creacion."', subtotal_pedido=".$total.", descuento_pedido=".$descuento." where numero_pedido=".$_GET['id_factura']."");
				if (!$insert_tmp) {
					$mensaje  = 'Consulta no válida: ' .mysqli_error($con) . "\n";
					echo '<script> M.toast({html: "Internal Server Error: Error en almacenar los  producto para el pedido: '.$_GET["id_factura"].'",classes: "error",displayLength:4000,inDuration:700,outDuration:700});</script>';
				}else{
					...
					$delete=mysqli_query($con, "DELETE FROM tmp WHERE id_factura=".$_GET["id_factura"]."");
...
<?php
	...
	if(isset($_GET['id_factura']))
		$sql=mysqli_query($con, "select tmp.cantidad_tmp as catidadtotal,tmp.valor_promo, tmp.disminuir_promo,tmp.ver_promo, productos.capacidad, productos.polaridad,tmp.id_tmp,tmp.status, tmp.id_producto, tmp.session_id, productos.nombreProducto, tmp.precio_tmp, productos.iva, productos.promocion, productos.codigoLinea, productos.codigoListaPrecio, tmp.promo,nombreLinea from productos, tmp, listalinea where productos.idProducto=tmp.id_producto and tmp.promo!=1 and productos.codigoLinea=listalinea.codigoLinea and tmp.id_factura=".$_GET['id_factura']."  order by productos.orden ASC");
			...
			while ($row=mysqli_fetch_array($sql))
				{
					...
					$update=mysqli_query($con,"UPDATE tmp SET descuento_unitario=".$valor.", descuento_iva=".$valor2." WHERE id_producto=".$row["id_producto"]." and id_factura=".$_GET['id_factura']."");
					if (!$update) {
						$mensaje  = 'Consulta no válidas: ' .mysqli_error($con) . "\n";
						echo $mensaje;
						return '';
