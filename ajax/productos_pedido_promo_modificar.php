<?php
	...
	if(isset($_SESSION['maxPrecioPromo']))
		$maxPrecioPromo=str_replace(",","",$_SESSION['maxPrecioPromo']);
	else
		$maxPrecioPromo=999999;
	...
	if ( isset($_GET['q']) != "" ){
		...
		$sTable = "productos,listalinea,clientelinea,clientes";
		$sWhere = "WHERE clientelinea.codigoCliente=clientes.codigoCliente and clientes.codigoCliente='".$_SESSION['user_id']."' and listalinea.codigoLinea=clientelinea.codigoLinea and clientelinea.estado=0 and listalinea.codigoLinea=productos.codigoLinea and 
		productos.estadoProd=1 and productos.facturar=1 and productos.web=1 and productos.promocion=1 and productos.precioUnitario>0 and productos.precioUnitario <= ".$maxPrecioPromo."";
	...
	$sWherenum =$sWhere;
		...
		if ($numrows>0){
			...
			<div class="table-responsive" >
				...
				<tbody style=" height:auto; max-height:950px; min-height:auto; width:100%">
					<tr class="" style="background-color:#faebcc!important; ">
						...
						while ($row=mysqli_fetch_array($query)){
								...
								<tr >
									...
									<td  style="width:10%;text-align:center;"><div class="pull-right">
										<input type="number" min="1" onkeyup="el3('<?php echo $id_producto; ?>')"  class="form-control" style="text-align:center" id="cantidad_<?php echo $id_producto; ?>"  value="1" >
									</div></td>
									<td  style="width:10%;text-align:center;" ><div class="pull-right">
										<input type="text" class="form-control"  style="text-align:center" readonly disabled  id="precio_venta_<?php echo $id_producto; ?>"  value="<?php echo $precio_venta;?>" >
									</div></td>
									<td style="width:10%;" class='text-center'><a   class='agregar'href="#" onclick="agregarpromomodificar('<?php echo $id_producto ?>')"><i  class="glyphicon glyphicon-plus"></i></a></td>
								</tr>
								<?php
							$i=$i+1;
						}
			...
			<?php
				...
				if($count<=0){
					...
					Cliente con cedula: <?php echo $_SESSION['user_id']; ?> ha agregado el numero de productos de promoción permitidos
					</div>
				<?php
